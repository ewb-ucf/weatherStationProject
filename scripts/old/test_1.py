...
GPIO.setup(7,GPIO.OUT)  # Set it as an output so that we can:
GPIO.output(7,GPIO.HIGH) # write a 1
time.sleep(0.025) # for 25 ms
GPIO.output(7,GPIO.LOW) # then write a 0
...
GPIO.setup(7, GPIO.IN, pull_up_down=GPIO.PUD_UP) # Change the pin to read mode, with a pullup resistor

for i in range(0,500): # 501 times
    data.append(GPIO.input(7)) # read a bit from the GPIO, as fast as possible (no wait)
